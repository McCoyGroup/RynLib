...
if [[ "$build_type" = "docker" ]]; then
  docker build -t $RYNLIB_IMAGE_NAME -f $RYNLIB_PATH/setup/build/Ubuntu/RynlibUpdate $RYNLIB_PATH
  if [[ "$push" == "--push" ]]; then
...
if [[ "$build_type" = "ompi" ]]; then
  docker build -t $RYNLIB_IMAGE_NAME-ompi -f $RYNLIB_PATH/setup/build/Ubuntu-OpenMPI/RynlibUpdate-OpenMPI $RYNLIB_PATH
  if [[ "$push" == "--push" ]]; then
...
if [[ "$build_type" = "singularity" ]]; then
  docker build -t $RYNLIB_IMAGE_NAME-centos -f $RYNLIB_PATH/setup/build/CentOS/RynlibUpdateCentOS $RYNLIB_PATH
  if [[ "$push" == "--push" ]]; then
